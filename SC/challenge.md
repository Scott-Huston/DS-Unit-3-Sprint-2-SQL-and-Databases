...
The relationship between the Employee and Territory tables is one-to-many. Each employee has many territories that they cover, but each territory is only covered by one employee.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

A document store is the best choice for situations that require extremely large amounts of data. That is because document stores allow for horizontal scaling and SQl does not. At some amount of data stored, further vertical scaling becomes impractical and horizontal scaling becomes the best choice. That said, document stores do not have the same guarantees of Atomicity, Consistency, Isolation, and Durability (ACID) that SQL does. In general it is better to use a SQL structure for situations where SQL is practical from a size standpoint but to use a document store when working with very large amounts of data.

- What is "NewSQL", and what is it trying to achieve?

NewSQL is trying to achieve the ACID guarantees of SQL and combine them with the ability to scale horizontally. If it succeeds, it will combine the best properties of both SQL and document store approaches.
